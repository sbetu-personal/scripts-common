Below is an **updated Confluence page** template for **CBC-AWS-EBS-2** (EBS volume encryption) that includes an **additional section** detailing what **IAM roles or permissions** are needed to perform the remediation steps. Feel free to adapt the roles/permissions to your organization’s naming conventions and policies.
...
2. **Attach or Launch**  
   ...
   - The snapshot remains **unencrypted** in your account unless you later remove or re-copy it.

**Pros**: Quick and easy if you only need the final volume encrypted.  
**Cons**: You still have an unencrypted snapshot in your environment unless you later delete it or re-copy it.
...
Because a new sub-team will handle these remediations, ensure they have (at minimum) the permissions below. You can grant these via an IAM role (preferred) or an IAM policy attached to a user/group.

**Recommended Permissions**:
1. **EC2 Volume & Snapshot Management**  
   - `ec2:CreateVolume`, `ec2:DeleteVolume`, `ec2:AttachVolume`, `ec2:DetachVolume`,  
   - `ec2:CreateSnapshot`, `ec2:DeleteSnapshot`, `ec2:CopySnapshot`, `ec2:ModifySnapshotAttribute`,  
   - `ec2:DescribeVolumes`, `ec2:DescribeSnapshots`, etc.  
   - This set allows them to identify unencrypted volumes, create snapshots, copy snapshots with encryption, and replace volumes.

2. **KMS Key Access**  
   - `kms:Encrypt`, `kms:Decrypt`, `kms:CreateGrant`, `kms:DescribeKey`,  
   - Some teams allow broader KMS actions such as `kms:GenerateDataKey` if needed.  
   - They need access to the relevant **CMK** (Customer Managed Key) or the AWS-managed key to encrypt/decrypt volumes.

3. **EC2 Instance Management** (Optional)  
   - If they need to **stop**, **start**, or **attach/detach** volumes to running instances, consider permissions like `ec2:StopInstances`, `ec2:StartInstances`, `ec2:DescribeInstances`.

4. **IAM PassRole** (If Using Terraform or EC2 with roles)  
   - If your Terraform or automation uses a specific role to perform these actions, the sub-team might need `iam:PassRole` permission to allow the service or pipeline to assume that role.

**Implementation**:  
- Create an **IAM role** (e.g., `EBS-Encryption-RemediationRole`) with the above permissions.  
- Attach that role to the sub-team’s **AWS profile** or let them assume it via AWS STS (AWS Single Sign-On, etc.).  
- Ensure the scope is limited to the accounts/regions they’ll be remediating.
